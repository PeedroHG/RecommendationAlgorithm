...
#include <iostream>
#include <algorithm>
#include <set> // Para candidatos únicos na avaliação LSH
#ifdef _OPENMP
#include <omp.h>
#endif

// evaluateKNNLeaveOneOut (versão OpenMP anterior) permanece a mesma
void evaluateKNNLeaveOneOut(const UserItemMatrix& user_item_matrix, int K_neighbors_for_eval) {
    ...
    #pragma omp parallel
    {
        ...
        for (size_t i = 0; i < user_matrix_vec.size(); ++i) {
            ...
            for (const auto& test_item_entry : current_user_all_ratings) {
                ...
                NeighborList neighbors_for_test_item; // KNN exato para avaliação
    ...
    for(int i=0; i < num_threads_to_merge; ++i) {
        final_squared_errors.insert(final_squared_errors.end(), thread_local_squared_errors[i].begin(), thread_local_squared_errors[i].end());
        final_absolute_errors.insert(final_absolute_errors.end(), thread_local_absolute_errors[i].begin(), thread_local_absolute_errors[i].end());
        final_total_predictions_made += thread_local_predictions_made[i];
    }

    if (final_total_predictions_made == 0) {
        std::cout << "⚠️ Insufficient data or no predictions made during KNN Leave-One-Out evaluation." << std::endl;
        return;
    }
    ...
    std::cout << "\n📊 K-NN Leave-One-Out Evaluation (K=" << K_neighbors_for_eval << ") [Exact KNN - Parallel]:" << std::endl;
    std::cout << "➡️ RMSE: " << rmse << std::endl;
    std::cout << "➡️ MAE : " << mae << std::endl;
    std::cout << "🔢 Total predictions: " << final_total_predictions_made << std::endl;
}


// Nova função para avaliar LSH
void evaluateLSHLeaveOneOut(
    const UserItemMatrix& user_item_matrix, // Dados completos
    const UserNormsMap& original_user_norms, // Normas dos dados completos
    const std::vector<HyperplaneSet>& all_hyperplane_sets, // Hiperplanos LSH
    const std::vector<LSHBucketMap>& lsh_tables, // Buckets LSH dos dados completos
    ...
    #pragma omp parallel
    {
        ...
        for (size_t i = 0; i < user_matrix_vec.size(); ++i) {
            ...
            if (current_user_all_ratings.size() < 2) continue;
            ...
            for (const auto& test_item_entry : current_user_all_ratings) {
                ...
                // Encontra candidatos usando LSH para o perfil temporário do usuário
                std::set<int> candidate_user_ids_for_test;
                for (size_t table_idx = 0; table_idx < lsh_tables.size(); ++table_idx) {
                    LSHHashValue temp_user_hash = computeLSHHash(current_user_ratings_copy, all_hyperplane_sets[table_idx], movie_to_idx);
                    
                    const auto& current_bucket_map = lsh_tables[table_idx];
                    auto bucket_it = current_bucket_map.find(temp_user_hash);
                    if (bucket_it != current_bucket_map.end()) {
                        for (int candidate_id : bucket_it->second) {
                            if (candidate_id != current_user_id) { // Não compara com ele mesmo
                                candidate_user_ids_for_test.insert(candidate_id);
                            }
                        }
                    }
                }
                ...
                for (int candidate_id : candidate_user_ids_for_test) {
                    // Calcula similaridade exata entre o perfil temporário do usuário atual e o perfil completo do candidato
                    const auto& candidate_ratings = user_item_matrix.at(candidate_id); // Perfil completo do candidato
                    float candidate_norm = original_user_norms.at(candidate_id); // Norma completa do candidato

                    float similarity = calculateCosineSimilarity(current_user_ratings_copy, candidate_ratings,
                                                                 norm_for_temp_profile, candidate_norm);
                    if (similarity > 0.0f) {
                        neighbors_for_test_item.emplace_back(candidate_id, similarity);
                    }
                }
                ...
                float predicted_rating_denominator = 0.0f;
                for (const auto& neighbor : neighbors_for_test_item) {
                    // O vizinho (neighbor.first) usa seu perfil completo para verificar se ele avaliou test_movie_id
                    const auto& neighbor_all_ratings = user_item_matrix.at(neighbor.first);
                    auto it_neighbor_rating = neighbor_all_ratings.find(test_movie_id);
                    if (it_neighbor_rating != neighbor_all_ratings.end()) {
                        predicted_rating_numerator += it_neighbor_rating->second * neighbor.second; // neighbor.second é a similaridade
                        predicted_rating_denominator += neighbor.second;
                    }
                }
    ...
    if (final_total_predictions_made == 0) {
        std::cout << "⚠️ Insufficient data or no predictions made during LSH Leave-One-Out evaluation." << std::endl;
        return;
    }
    double sum_sq_err = std::accumulate(final_squared_errors.begin(), final_squared_errors.end(), 0.0);
    double sum_abs_err = std::accumulate(final_absolute_errors.begin(), final_absolute_errors.end(), 0.0);
    double rmse = std::sqrt(sum_sq_err / final_total_predictions_made);
    double mae = sum_abs_err / final_total_predictions_made;
    std::cout << "\n📊 LSH Leave-One-Out Evaluation (K=" << K_neighbors_for_eval 
              << ", L=" << NUM_LSH_TABLES << ", k_hash=" << NUM_HYPERPLANES_PER_TABLE << ") [Parallel]:" << std::endl;
    std::cout << "➡️ RMSE: " << rmse << std::endl;
    std::cout << "➡️ MAE : " << mae << std::endl;
    std::cout << "🔢 Total predictions: " << final_total_predictions_made << std::endl;
}