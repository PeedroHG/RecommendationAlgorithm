...
OfflineEvalResult evaluate_precision_recall(const std::vector<int>& recommendations, const std::unordered_set<int>& test_set) {
    ...
    for (int rec : recommendations) {
        if (test_set.count(rec)) hits++;
    }
    double precision = recommendations.empty() ? 0.0 : (double)hits / recommendations.size();
    double recall = test_set.empty() ? 0.0 : (double)hits / test_set.size();
